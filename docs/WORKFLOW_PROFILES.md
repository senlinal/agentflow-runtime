...
Role Timeline entries are runtime-verified. They are extracted from `trace.json` and marked `source=runtime_trace`; the formatter does not display roles inferred from OpenCode supervisor text or model prose. To inspect the visible multi-agent workforce directly:
