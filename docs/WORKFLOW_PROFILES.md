...
The CLI text output is formatted for users. It shows `AgentFlow Profile Run`, routing, autonomy, `Runtime Proof`, `AgentFlow Role Timeline`, artifact paths, warnings, and next actions. opencode `/workflow` should prefer the `run_profile_workflow` tool and display its `formattedText`; if the tool is unavailable, it should fall back to `npm run workflow:run-profile -- --task "<task>"`.

Role Timeline entries are runtime-verified. They are extracted from `trace.json` and marked `source=runtime_trace` or `source=subagent_dispatch_trace`; the formatter does not display roles inferred from OpenCode supervisor text or model prose. To inspect the visible multi-agent workforce directly:
